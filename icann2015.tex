...
\author{
Qian~Liu,~Steve~Furber%,~\IEEEmembership{Fellow,~IEEE}
\thanks{
Qian Liu and Steve Furber are with the School of Computer Science, University of Manchester, Manchester M13 9PL, U.K. 
...
There are attempts~\cite{la2008response}~\cite{burkitt2006review} to estimate the output firing rate of the LIF neurons~(\ref{equ:lif}) under certain conditions. 
...
\label{equ:sgt}
\end{align}
where $\tau_Q, \mu_Q, \sigma_Q$ are identical to $\tau_m, \mu, \sigma$ in~(\ref{equ:ou}), and erf is the error function.
